...
| `migrations/0008_copy_pilot_log.sql` | Stage 3 — copies 325 timeslots and 395 events from `public` into `app`, remapping `baby_id` onto the existing `app.baby` and `logged_by` onto the caregiver of the same name. Carries `baby.settings` across. **Reads `public`, never writes it** | **Not yet applied.** Take the JSON export first. Refuses to run on an ambiguous baby, a caregiver name that does not map one-to-one, or a surviving `grams`/`celsius` value; refuses a short copy and rolls back. Safe to re-run. D-060 |
