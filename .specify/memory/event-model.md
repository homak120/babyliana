...
| | Holds | Why there |
| --- | --- | --- |
| **`localStorage`** | The device id. Nothing else | The one fact that is genuinely per-device and must *not* sync |
...
**The device id is created on submit, never on read.** An earlier version minted
one the first time anything asked for it, so merely opening the URL inserted a
device row — a phantom identity for anyone who glanced at the page. Nothing
exists until someone commits to a name.

That also removed the separate "welcomed" flag: the id's *presence* is what says
setup has happened, so there are no longer two facts that can drift apart.
