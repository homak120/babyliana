...
## D-065 — the launch is the app's colour, not white

**2026-09-25. Owner's report: "a few seconds of white", and it was measured
before it was fixed.**

**What the measurement said.** On the real bundle with a 60-day log seeded — 540
timeslots, 1,080 events, larger than the live log:

| | phone-ish CPU (6×) | slow phone (20×) |
| --- | --- | --- |
| IndexedDB open + read + join | **6 ms** | 6 ms |
| first paint | 60 ms | 232 ms |
| first contentful paint | 116 ms | 448 ms |

**So none of the seconds were the app's own boot**, and an earlier answer in this
repo's session log blaming IndexedDB was wrong: reading the whole log is 6 ms.
The font stylesheets answer from the service worker cache in 2 ms even with a
two-second network delay injected. The page's own window is under half a second
on a CPU throttled twenty times.

The seconds are **before the page timeline starts** — iOS bringing the web view
up — and what is on screen for them is the OS launch screen.

### Four changes, in the order they matter

1. **iOS gets a launch screen.** `index.html` had no `apple-touch-startup-image`
   at any size, and without one an installed iOS web app draws **white** for the
   whole launch; the manifest's `background_color` does not reliably stand in.
   Twelve portrait sizes, generated by `scripts/splash.mts`, flat ground and no
   mark — 172 KB for the set, against 2.1 MB when the app's icon was on them,
   because the art is photographic and PNG charges for that. Apple's own guidance
   arrives at the same place from the other end: a launch screen should resemble
   the app's first screen rather than carry a logo. **Not precached** — the OS
   reads them at install, and install happens online, the same argument the 1024
   icon already won.
2. **The page paints its ground on the first frame.** `html` carried no
   background at all, so the canvas was the UA's white until the bundle's CSS
   arrived — behind three render-blocking stylesheets, two of them on a host in
   another country. An inline `<style>` and a three-line script now set the
   ground before any stylesheet resolves. The script reads the clock rather than
   `prefers-color-scheme`, because this app's theme is the clock's (`themeFor`,
   20:00–07:00) and the system setting would flash the wrong one every evening.
3. **The body font is self-hosted and precached** — 64 KB of latin and latin-ext
   across four weights, declared inline, with the two common weights preloaded.
   The two Google stylesheets stay, loaded without blocking the paint, and keep
   what the subset does not: every non-latin glyph and the whole icon set.
   **The icon font is deliberately not subsetted**: icon names are passed as
   variables in a dozen places, and a name outside a subset renders as its own
   literal text in the middle of the UI.
4. **The first load is the home screen only.** The report, its charts, the
   period picker and the two diagnostics are behind `lazy`. The day screen's
   33 KB of JS and the insights screen's 8 KB of CSS leave the first load; what
   stays is React, the home screen and the Supabase client.

   **`day.css` does not travel with them**, and that cost a build to learn. It is
   named for the day screen and it also styles `.tabs` — the bottom bar `App`
   renders on *every* screen, the FAB, and the end-feed and end-sleep pills. In
   the lazy chunk, the whole bar rendered as unstyled 29 px buttons until someone
   opened the report. `verify-sleep` caught it by asserting the resolved layout
   of one pill rather than its presence, which is exactly why that check was
   written that way.

### What it measured afterwards, including the part that got worse

Same rig, three runs each:

| | before | after |
| --- | --- | --- |
| first paint (6× CPU) | 60 ms | ~82 ms |
| first contentful paint (6× CPU) | 116 ms | ~205 ms |
| render-blocking stylesheets | 3, two cross-origin | **1, local** |
| initial JS | 508 KB | 481 KB + 33 KB on the report |
| initial CSS | 48 KB | 40 KB + 8 KB on the report |

**The warm boot is slower, not faster, and change 2 does not do what its name
suggests.** An inline `<style>` cannot paint before a render-blocking
stylesheet, and the bundle's own CSS is still one — so on a warm cache the
ground arrives exactly when it always did. The extra ~90 ms of first contentful
paint is the webfont's block period, now spent on a font that will certainly
arrive rather than on one that might not.

Both figures are far below noticing, and neither is where the seconds were. What
the changes actually buy is the tail: **nothing on the critical path is
cross-origin any more.** Before, a cold service-worker cache — the first launch
after any deploy — meant first paint waited on two stylesheets from another
network. That is the case that could take seconds, and it is now impossible.
Recorded rather than smoothed over, because a decision that claims a speed-up it
did not get is a decision that gets built on.

### What was not done, and why

**The Supabase client is still on the critical path** — 210 KB, pulled in
because `App` starts sync on mount. Moving it means splitting sync's listener
state from its network half so the state can be imported cheaply and the network
half loaded dynamically. That is a refactor of the code D-039 was written about,
and the payoff is tens of milliseconds against a first paint already at 60 ms.
Named here rather than left as an oversight.

**One limitation of the launch screens.** iOS picks a startup image by media
query, and this app's theme follows the clock, so a night launch shows the day
ground and then goes dark. That is a colour changing where it used to be white,
which is the whole complaint; a `prefers-color-scheme` variant set would help
only the phones that are on automatic dark mode.

### One booby trap, found by the suites

The first version of the comment above this block in `index.html` contained the
head's own closing tag, spelled out, in a sentence. **Vite injects the module
script before the first occurrence of that string in the file** — it found the
one inside the comment, and emitted a page whose entire application was
commented out. The build succeeded, the HTML looked right to a reader, and every
browser suite failed at once on a blank page with no console error.

Worth recording because the failure is invisible at both ends: nothing warns,
and the only symptom is the exact fault this decision set out to fix.

**Reversal condition.** If a future iOS draws the manifest's `background_color`
for launch on both phones, the twelve images stop earning their place and the
generator goes with them.

**And one thing this decision proves about the split.** `Welcome` was lazy for
one build — 6 KB, used once per install, the easiest-looking entry on the list.
It is eager again: the boot it is needed on is the first one, with no caches and
